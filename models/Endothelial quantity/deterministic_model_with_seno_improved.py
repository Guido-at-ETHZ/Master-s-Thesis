...
def calculate_delayed_toxicity(senolytic_history, current_time, time_points, k_clear=0.1):
    """
    Calculate the delayed toxicity integral: D_tox(t) = ∫₀ᵗ c(τ)e⁻ᵏᶜˡᵉᵃʳ⁽ᵗ⁻τ⁾ dτ

    Args:
        senolytic_history: Array of senolytic concentrations over time
        current_time: Current time point to calculate toxicity for
        time_points: Array of time points corresponding to senolytic_history
        k_clear: Clearance rate constant for toxicity
    """
    # Find the indices for time points up to current_time
    valid_indices = time_points <= current_time

    if not np.any(valid_indices):
        return 0  # No toxicity at the start

    # Extract valid time points and concentrations
    valid_times = time_points[valid_indices]
    valid_concs = senolytic_history[valid_indices]

    # Calculate time differences for integration
    if len(valid_times) > 1:
        dt = np.diff(valid_times)
        dt = np.append(dt, current_time - valid_times[-1])
    else:
        dt = np.array([current_time - valid_times[0]])

    # Calculate exponential decay factors
    decay_factors = np.exp(-k_clear * (current_time - valid_times))

    # Calculate integral using simple numerical integration
    integral = np.sum(valid_concs * decay_factors * dt)

    return integral


# Modified cell dynamics model to include delayed toxicity
def endothelial_cell_dynamics_with_delayed_toxicity(t, y, params):
    """
    Modified version of endothelial_cell_dynamics that includes delayed toxicity effects.
    """
    # Unpack parameters
    r = params['r']  # Base proliferation rate
    K = params['K']  # Carrying capacity
    ...
    # Extract delayed toxicity parameters
    senolytic_history = params.get('senolytic_history', None)
    time_points = params.get('time_points', None)
    k_clear = params.get('k_clear', 0.1)
    ...
    # Calculate delayed toxicity if history is provided
    D_tox = 0
    if senolytic_history is not None and time_points is not None:
        D_tox = calculate_delayed_toxicity(senolytic_history, t, time_points, k_clear)
    ...
    # Equations for healthy cells at each division stage
    for i in range(max_divisions + 1):
        # Age-dependent death rate (increases with division age) + MODIFIED to include delayed toxicity
        death_rate = d_E * (1 + 0.03 * i + 0.2 * D_tox)
...
# Run simulation with delayed toxicity (continuous treatment)
def run_simulation_with_delayed_toxicity(tau_value, senolytic_conc=0, max_divisions=15, initial_conditions=None,
                                         t_span=(0, 600), t_points=1000, k_clear=0.1):
    """
    Runs a simulation of the endothelial cell dynamics model with senolytic effects and delayed toxicity.
    """
    # Default initial conditions if none provided
    if initial_conditions is None:
        # Start with all cells in the undivided state (E_0)
    ...
    # Create senolytic history for continuous treatment (constant concentration)
    senolytic_history = np.ones(t_points) * senolytic_conc
    ...
    params = {
        ...
        'senolytic_conc': senolytic_conc,  # Concentration of senolytics
        'sen_efficacy_tel': 1.0,  # Efficacy of senolytics on telomere-induced senescent cells
        'sen_efficacy_stress': 1.2,  # Efficacy of senolytics on stress-induced senescent cells
        'senolytic_history': senolytic_history,  # History of senolytic concentrations
        'time_points': t_eval,  # Time points corresponding to concentration history
        'k_clear': k_clear,  # Clearance rate for toxicity
    }

    # Solve the system of differential equations
    solution = solve_ivp(
        lambda t, y: endothelial_cell_dynamics_with_delayed_toxicity(t, y, params),
        t_span,
        initial_conditions,
        t_eval=t_eval,
        method='RK45'
...
# Run simulation with pulsed senolytic treatment and delayed toxicity
def run_pulsed_treatment_simulation_with_delayed_toxicity(tau_value, senolytic_conc=10, pulse_interval=50,
                                                          pulse_duration=10, max_divisions=15, t_span=(0, 600),
                                                          t_points=1000, k_clear=0.1):
    """
    Runs a simulation with pulsed senolytic treatment and delayed toxicity.
    """
    # Define the final time points we want to evaluate at
    t_eval = np.linspace(t_span[0], t_span[1], t_points)
    ...
    params = {
        ...
        'sen_efficacy_tel': 1.0,
        'sen_efficacy_stress': 1.2,
        'senolytic_history': senolytic_history,
        'time_points': t_eval,
        'k_clear': k_clear,
    }

    # Create a modified version of endothelial_cell_dynamics that takes time-varying senolytic concentration
    def dynamics_with_time_varying_senolytics(t, y):
        # Find the closest index in t_eval
        idx = np.argmin(np.abs(t_eval - t))
        # Set the senolytic concentration for this time point
        params_copy = params.copy()  # Create a copy to avoid modifying the original
        params_copy['senolytic_conc'] = senolytic_history[idx]
        # Call the dynamics function with delayed toxicity
        return endothelial_cell_dynamics_with_delayed_toxicity(t, y, params_copy)

    # Solve the system of differential equations
    solution = solve_ivp(
        dynamics_with_time_varying_senolytics,
        t_span,
        initial_conditions,
        t_eval=t_eval,
        method='RK45'
...
# Function to compare continuous vs. pulsed senolytic treatment with delayed toxicity
def compare_treatments_with_delayed_toxicity(tau_value, max_divisions=15, k_clear=0.1):
    """
    Compares continuous and pulsed senolytic treatments with and without delayed toxicity.
    """
    # Find optimal senolytic concentration
    opt_conc, _ = optimize_senolytic_concentration(tau_value, max_divisions)
    ...
    # Run simulations without delayed toxicity
    t_cont, sol_cont, _ = run_simulation(
        tau_value, senolytic_conc=opt_conc, max_divisions=max_divisions
    )

    t_pulse, sol_pulse, _ = run_pulsed_treatment_simulation(
        tau_value, senolytic_conc=pulsed_conc,
        pulse_interval=pulse_interval, pulse_duration=pulse_duration,
        max_divisions=max_divisions
    )

    # Run simulations with delayed toxicity
    t_cont_tox, sol_cont_tox, _ = run_simulation_with_delayed_toxicity(
        tau_value, senolytic_conc=opt_conc,
        max_divisions=max_divisions, k_clear=k_clear
    )

    t_pulse_tox, sol_pulse_tox, _ = run_pulsed_treatment_simulation_with_delayed_toxicity(
        tau_value, senolytic_conc=pulsed_conc,
        pulse_interval=pulse_interval, pulse_duration=pulse_duration,
        max_divisions=max_divisions, k_clear=k_clear
    )
    ...
    # Analyze results
    res_cont = analyze_results(t_cont, sol_cont, max_divisions)
    res_pulse = analyze_results(t_pulse, sol_pulse, max_divisions)
    res_cont_tox = analyze_results(t_cont_tox, sol_cont_tox, max_divisions)
    res_pulse_tox = analyze_results(t_pulse_tox, sol_pulse_tox, max_divisions)
    ...
    # Plot healthy cells
    ax = axes[0, 0]
    ax.plot(res_cont['t'], res_cont['E_total'], 'b-', label='Continuous (No Tox)')
    ax.plot(res_pulse['t'], res_pulse['E_total'], 'r-', label='Pulsed (No Tox)')
    ax.plot(res_cont_tox['t'], res_cont_tox['E_total'], 'b--', label='Continuous (With Tox)')
    ax.plot(res_pulse_tox['t'], res_pulse_tox['E_total'], 'r--', label='Pulsed (With Tox)')
    ax.set_xlabel('Time')
    ax.set_ylabel('Healthy Cell Count')
    ax.set_title('Healthy Cell Population')
    ax.legend()
    ...
    # Plot senescent cells
    ax = axes[0, 1]
    ax.plot(res_cont['t'], res_cont['S_total'], 'b-', label='Continuous (No Tox)')
    ax.plot(res_pulse['t'], res_pulse['S_total'], 'r-', label='Pulsed (No Tox)')
    ax.plot(res_cont_tox['t'], res_cont_tox['S_total'], 'b--', label='Continuous (With Tox)')
    ax.plot(res_pulse_tox['t'], res_pulse_tox['S_total'], 'r--', label='Pulsed (With Tox)')
    ax.set_xlabel('Time')
    ax.set_ylabel('Senescent Cell Count')
    ax.set_title('Senescent Cell Population')
    ax.legend()
    ...
    # Plot senescent fraction
    ax = axes[1, 0]
    ax.plot(res_cont['t'], res_cont['senescent_fraction'], 'b-', label='Continuous (No Tox)')
    ax.plot(res_pulse['t'], res_pulse['senescent_fraction'], 'r-', label='Pulsed (No Tox)')
    ax.plot(res_cont_tox['t'], res_cont_tox['senescent_fraction'], 'b--', label='Continuous (With Tox)')
    ax.plot(res_pulse_tox['t'], res_pulse_tox['senescent_fraction'], 'r--', label='Pulsed (With Tox)')
    ax.set_xlabel('Time')
    ax.set_ylabel('Fraction')
    ax.set_title('Fraction of Senescent Cells')
    ax.set_ylim(0, 1)
    ...
    # Calculate delayed toxicity over time
    t_points = len(t_cont)
    cont_tox = np.zeros(t_points)
    pulse_tox = np.zeros(t_points)

    # Create senolytic histories
    cont_history = np.ones(t_points) * opt_conc

    pulse_history = np.zeros(t_points)
    for i, t in enumerate(t_cont):
        is_pulse_period = (t % pulse_interval) < pulse_duration
        pulse_history[i] = pulsed_conc if is_pulse_period else 0

    # Calculate toxicity for each time point
    for i, t in enumerate(t_cont):
        cont_tox[i] = calculate_delayed_toxicity(cont_history, t, t_cont, k_clear)
        pulse_tox[i] = calculate_delayed_toxicity(pulse_history, t, t_cont, k_clear)

    # Plot delayed toxicity
    ax = axes[1, 1]
    ax.plot(t_cont, cont_tox, 'b-', label='Continuous')
    ax.plot(t_cont, pulse_tox, 'r-', label='Pulsed')
    ax.set_xlabel('Time')
    ax.set_ylabel('Delayed Toxicity (D_tox)')
    ax.set_title('Accumulated Toxicity Over Time')
    ax.legend()
    ax.grid(True)

    plt.suptitle(f'Comparison of Treatment Strategies with Delayed Toxicity (τ={tau_value}, k_clear={k_clear})',
                 fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])

    return fig, res_cont_tox, res_pulse_tox
...
if __name__ == "__main__":
    ...
    # Phase 5: Testing delayed toxicity model
    print("Phase 5: Testing delayed toxicity model...")
    # Try different clearance rates
    for k_clear in [0.05, 0.1, 0.2]:
        for tau in tau_values:
            fig, res_cont, res_pulse = compare_treatments_with_delayed_toxicity(tau, max_divisions, k_clear)
            fig.savefig(f'delayed_toxicity_comparison_tau_{tau}_k_{k_clear}.png')
            plt.close(fig)

            # Print key metrics
            print(f"For τ={tau}, k_clear={k_clear}:")
            print(f"  Final healthy/senescent ratio:")
            print(f"    Continuous with toxicity: {res_cont['E_total'][-1]:.1f}/{res_cont['S_total'][-1]:.1f}")
            print(f"    Pulsed with toxicity: {res_pulse['E_total'][-1]:.1f}/{res_pulse['S_total'][-1]:.1f}")
            print(
                f"  Difference: {((res_pulse['E_total'][-1] - res_cont['E_total'][-1]) / max(res_cont['E_total'][-1], 1) * 100):.1f}%")