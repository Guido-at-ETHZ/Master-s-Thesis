...
def endothelial_cell_dynamics(t, y, params):
    ...
    # New parameters for stem cell input
...
def run_simulation(tau_value, max_divisions=15, stem_cell_rate=5, distribution_type='exponential',
                   peak_stage=0, width=2, initial_conditions=None, t_span=(0, 600), t_points=1000):
    """
    Runs a simulation of the endothelial cell dynamics model with stem cell input.
    """
    ...
    stem_cell_distribution = create_stem_cell_distribution(
        max_divisions, distribution_type, peak_stage, width
    )
...
# Function to analyze results - FIXED implementation
...
# Run simulations with and without stem cell input for comparison
def run_comparison_simulations(tau_value=10, max_divisions=15, stem_cell_rates=[0, 5, 10],
                               distribution_type='exponential', peak_stage=0, width=2,
                               t_span=(0, 1000), t_points=1000):
    """
    Run simulations with different stem cell input rates for comparison.
    """
    results = []

    for rate in stem_cell_rates:
        ...
        result['stem_cell_rate'] = rate
        result['distribution_type'] = distribution_type
        result['peak_stage'] = peak_stage
        result['tau'] = tau_value
        results.append(result)

    return results


# Plot the simulation results with stem cell input - IMPROVED
def plot_stem_cell_comparison(results, max_divisions=15):
    """
    Plot results comparing different stem cell input rates.
    """
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))

    # Colors for different stem cell rates
    ...
    for i, result in enumerate(results):
        ax.plot(result['t'], result['total_cells'], color=colors[i],
                label=f'Stem rate={result["stem_cell_rate"]}')
    ax.set_xlabel('Time')
    ...
    plt.suptitle('Effect of Stem Cell Input on Endothelial Cell Dynamics', fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])

    return fig


# Plot the original simulation results with different shear stress values
def plot_original_results(tau_values=[0, 5, 10, 15, 20], max_divisions=15):
    """
    Plot comprehensive results from multiple simulations with different shear stress values.
    This recreates the original plots from the vanilla model for comparison.
    """
    results = []

    for tau in tau_values:
        t_eval, solution, _ = run_simulation(tau, max_divisions=max_divisions, stem_cell_rate=0)
        result = analyze_results(t_eval, solution, max_divisions)
        result['tau'] = tau
        results.append(result)
    ...
    # Plot average division age - IMPROVED
    ax = axes[2, 1]
    for i, result in enumerate(results):
        # Handle potential NaN values
        mask = ~np.isnan(result['avg_division_age'])
    ...
    t_eval, solution, _ = run_simulation(tau_to_show, max_divisions=max_divisions, stem_cell_rate=0)
...
# Plot distribution of cells by division stage for different stem cell distributions
def plot_division_distribution_comparison(tau_value=10, max_divisions=15, stem_cell_rate=10,
                                          distribution_types=['exponential', 'gaussian', 'single_stage'],
                                          peak_stages=[0, 5, 10], time_point=600):
    """
    Compare the division distribution of healthy cells for different stem cell distribution patterns.
    """
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))

    # For each distribution type
    for i, (dist_type, peak) in enumerate(zip(distribution_types, peak_stages)):
        ax = axes[i]
        ...
        # Plot
        ax.bar(division_ages, normalized_distribution, color='blue')
        ax.set_xlabel('Division Stage')
        ax.set_ylabel('Percentage of Healthy Cells')
        ax.set_title(f'{dist_type.capitalize()}, peak={peak}, time={time_point}')
        ax.grid(True)
...
# Plot the stem cell distribution patterns
def plot_stem_cell_distributions(max_divisions=15):
    """
    Plot the different stem cell distribution patterns.
    """
    distribution_types = ['exponential', 'gaussian', 'single_stage', 'uniform']
    peak_stages = [0, 5, 10, 0]
    widths = [2, 2, 0, 0]

    plt.figure(figsize=(12, 6))

    for i, (dist_type, peak, width) in enumerate(zip(distribution_types, peak_stages, widths)):
        distribution = create_stem_cell_distribution(
            max_divisions, dist_type, peak, width
        )
        plt.plot(np.arange(max_divisions + 1), distribution,
                 label=f'{dist_type.capitalize()}, peak={peak}',
                 marker='o')

    plt.xlabel('Division Stage')
    plt.ylabel('Proportion')
    plt.title('Stem Cell Input Distribution Patterns')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
# Compare time evolution of distribution patterns
def plot_distribution_evolution(tau_value=10, max_divisions=15, stem_cell_rate=10,
                                distribution_type='exponential', peak_stage=0,
                                time_points=[100, 300, 600, 1000]):
    """
    Plot how the distribution of cells by division stage evolves over time.
    """
    ...
    for i, t in enumerate(time_points):
        ...
        # Get distribution at this time
        distribution = solution.y[:max_divisions + 1, t_idx]
        division_ages = np.arange(max_divisions + 1)

        # Normalize to percentage
        total_healthy = np.sum(distribution)
        if total_healthy > 0:
            normalized_distribution = 100 * distribution / total_healthy
        else:
            normalized_distribution = np.zeros_like(distribution)

        # Plot
        ax.bar(division_ages, normalized_distribution)
        ax.set_xlabel('Division Stage')
        ax.set_ylabel('Percentage of Healthy Cells')
        ax.set_title(f'Time = {t_eval[t_idx]:.1f}')
        ax.grid(True)

    plt.suptitle(
        f'Evolution of Cell Distribution - {distribution_type.capitalize()}, peak={peak_stage}, stem rate={stem_cell_rate}',
        fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
...
if __name__ == "__main__":
    # Run simulations with different stem cell input rates
    max_divisions = 15
    stem_cell_rates = [0, 5, 10, 20]

    print("Running simulations with different stem cell input rates...")
    results_rate_comparison = run_comparison_simulations(
        tau_value=10,
        max_divisions=max_divisions,
        stem_cell_rates=stem_cell_rates,
        distribution_type='exponential',
        t_span=(0, 1000)
    )

    # Plot results
    fig_rate = plot_stem_cell_comparison(results_rate_comparison, max_divisions)
    fig_rate.savefig('stem_cell_rate_comparison.png', dpi=300)

    # Plot original model results for comparison
    fig_original = plot_original_results(tau_values=[0, 5, 10, 15, 20], max_divisions=max_divisions)
    fig_original.savefig('original_model_results.png', dpi=300)

    # Plot different stem cell distribution patterns
    fig_dist_patterns = plot_stem_cell_distributions(max_divisions)
    fig_dist_patterns.savefig('stem_cell_distribution_patterns.png', dpi=300)

    # Plot division distribution comparison
    fig_division_dist = plot_division_distribution_comparison(
        distribution_types=['exponential', 'gaussian', 'single_stage'],
        peak_stages=[0, 5, 10],
        time_point=600
    )
    fig_division_dist.savefig('division_distribution_comparison.png', dpi=300)

    # Plot how distribution evolves over time for one case
    fig_evolution = plot_distribution_evolution(
        distribution_type='exponential',
        time_points=[100, 300, 600, 1000]
    )
    fig_evolution.savefig('distribution_evolution.png', dpi=300)
